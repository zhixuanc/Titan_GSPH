...
inline bool compare_keys ( const Key  & K1, const Key & K2)
{
...
class HashTable
{
  ...
  public:
    ...
    HashEntryPtr getBucket(int entry){return bucket[entry];}
    void* get_value();

    // Keith's modifications for better hashing
    double* get_minDom()     {return minDom;}
    double* get_maxDom()     {return maxDom;}
    double* get_invrange()     {return invrange;}
    double* get_keyrange()   {return keyrange;}
    unsigned* get_MinKey()   {return MinKey;}
    unsigned* get_MaxKey()   {return MaxKey;}
};
