...
void search_bnd_images(int myid, HashTable *P_table, HashTable *BG_mesh, 
                       vector<BndImage> *Image_table, int reset)
{
  ...
  if ( reset )
  {
    ...
    while ((p_temp = (Particle *) p_itr->next()))
      p_temp->set_have_reflection(false);
    delete p_itr;
  }
    ...
    if ( buck->have_ghost_particles() && !buck->is_guest() )
    {
      ...
      if ( plist.size() > 0 )
        for (p_itr=plist.begin(); p_itr!=plist.end(); p_itr++)
        {
          ...
          if ( p_ghost->is_ghost() && !(p_ghost->have_reflection()) )
          {
            ...
            for (i=0; i<NEIGH_SIZE; i++)
              ...
              BndImage bndimg(buck2->get_myprocess(), myid, buck_key, ghst_key, refc);
              Image_table->push_back(bndimg);
              p_ghost->set_have_reflection(true);
            }
        }
    }
  delete itr;
