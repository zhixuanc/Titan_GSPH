 ...
 */
inline bool in_support ( 
    //! cartesian projections of interparticle distance
    const double *dx,
    //! support
    double h 
    )
...
/*!
 * unrotate() rotates the vector of state variables clockwise. 
 * It involves rotation of velocity vector and rotation of stress-tensor.
 * It is used to rotate Reimann solution to global coordinate system
 */
void unrotate (
    //! the vector of state variables
    double *u,    
...
FILE * debug_fopen();
inline void  debug_close(FILE *fp)
{
  fclose(fp);
}
