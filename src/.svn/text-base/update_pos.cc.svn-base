...
int update_pos(int myid, HashTable *P_table, HashTable *BG_mesh, 
               FluxProps *fluxprops, TimeProps *timeprops, int *lost)
{
  ...
  while ( curr_bucket = (Bucket *) it2->next() )
    if ( curr_bucket->is_active() )
    {
          ...
          if ( left_curr_bucket )
          {
            if ( !p_curr->is_guest() ) // should always have neighbors
            {
              ...
              else // if neighbor is a native ... 
              {
                ...
                {
                  neigh->put_have_real_particles (true);
                  adapt = 1;
                }
      ...
      if ( !my_realp.empty() )
        my_particles.insert(my_particles.end(), my_realp.begin(), my_realp.end());
      else
        curr_bucket->put_have_real_particles (false);
