				   ...
				   HashTable* P_table, HashTable* BG_mesh)
{
  ...
  while ((buck = (Bucket *) itr->next()))
    if(myid != buck->get_myprocess())  // this element will get moved to a new processor
    {
      ...
      send_info[3*(buck->get_myprocess()) + 2] += (int) buck->get_plist().size();
    }
  ...
  i = MPI_Alltoall(temp_info, 3, MPI_INT, recv_info, 3, MPI_INT, MPI_COMM_WORLD);
  delete []temp_info;
  ...
  for(i=0;i<numprocs;i++) 
  {
    ...
    {
      j = MPI_Irecv((recv_neigh_array+counter_recv[0]), recv_info[3*i]*blksize,
                    MPI_UNSIGNED, i, neigh_tag, MPI_COMM_WORLD, (recv_request+3*i));
      counter_recv[0] += recv_info[3*i]*blksize;
    }
    if(recv_info[3*i+1] != 0)  // receive buckets
    {
      j = MPI_Irecv((recv_buck_array+counter_recv[1]), recv_info[3*i+1],
		    BUCKET_TYPE, i, buck_tag, MPI_COMM_WORLD, (recv_request+3*i+1));
      counter_recv[1] += recv_info[3*i+1];
    }
    if (recv_info[3*i+2] != 0) // receive particles
    {
      j = MPI_Irecv((recv_part_array+counter_recv[2]), recv_info[3*i+2],
    ...
    if(myid != buck->get_myprocess()) // this element will get moved to a new processor
    {
      ...
      for (j=0; j < NEIGH_SIZE; j++) 
        if( (neigh_proc[j] != myid) && (neigh_proc[j] > -1) )
        {
          ...
          {
            send_neigh_array[counter_send_proc[neigh_proc[j]]*blksize + k] =
              neighbors[j].key[k];
            send_neigh_array[counter_send_proc[neigh_proc[j]]*blksize + KEYLENGTH + k] =
              mykey.key[k];
          }
          send_neigh_array[counter_send_proc[neigh_proc[j]]*blksize + 2*KEYLENGTH] =
            (unsigned) buck->get_myprocess();
    ...
    if(myid != buck->get_myprocess()) 
    {
      ...
      for (i=0; i <= 2; i++)
        for(j=0; j <= 2; j++) 
        {
          ...
          if( neigh_proc == myid ) 
          {
            Bucket* BkNeigh = (Bucket *) BG_mesh->lookup(buck->which_neigh(dir1));
 ...
 ****************************************************/
  ...
  counter_send_proc[0] = 0;
  for (i=1; i < numprocs; i++)
    counter_send_proc[i] = counter_send_proc[i-1] + send_info[3*(i-1)+2];
    ...
    if ( myid != buck->get_myprocess() )
    {
      ...
      if ( plist.size() > 0 )
      {
        ...
        for ( ip = plist.begin(); ip != plist.end(); ip++ )
        {
          ...
          pack_particles(psend, (send_part_array+counter_send_proc[myprocess]));
  ...
  for(i=0; i < numprocs; i++)
    ...
    {
      j = MPI_Isend((send_part_array+counter), send_info[3*i+2], PARTICLE_TYPE,
		    i, part_tag, MPI_COMM_WORLD, (send_request+i));
  ...
  counter_send_proc[0] = 0;
  for(i=1;i<numprocs;i++)
    counter_send_proc[i] = counter_send_proc[i-1] + send_info[3*(i-1)+1];
  ...
  while ((buck = (Bucket *) itr->next())) 
    if(myid != buck->get_myprocess()) 
    {
      ...
      pack_bucket ((send_buck_array+counter_send_proc[myprocess]), buck, myprocess);
  ...
  for(i=0;i<numprocs;i++)
    ...
    {
      j = MPI_Isend((send_buck_array+counter), send_info[3*i+1], BUCKET_TYPE,
		    i, buck_tag, MPI_COMM_WORLD, (send_request+i));
