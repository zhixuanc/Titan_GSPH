 ...
 *
 *******************************************************************
 * $Id: update_element_info.C,v 1.1.1.1 2003/08/13 19:26:11 sorokine Exp $ 
 */
      ...
      for (i = 0; i < 8; i++)
        {
          if (r_element->neigh_proc[i] == myid)
            same_proc (r_element, HT_Elem_Ptr, target_proc, i);
                   ...
                   r_element->neigh_proc[i] != target_proc
           ...
           int target_proc, int side)
{
  Element *Neighbor;
  Neighbor =
    (Element *) HT_Elem_Ptr->lookup (r_element->get_neighbors () +
                                     side * KEYLENGTH);
      ...
      if (!node)
        {
          node = new Node (elem2->node_key[i], elem2->n_coord[i],
                           elem2->n_info[i], elem2->n_order[i],
                           elem2->node_elevation[i]);
  ...
  if (!node)
    {
      node = new Node (elem2->key, elem2->n_coord[8],
                       elem2->n_info[8], elem2->n_order[8],
                       elem2->node_elevation[8]);
  ...
  for (int i = 0; i < 8; i++)
    {
      if (*(neigh_proc + i) == myid)
        {
          neighbor =
            (Element *) HT_Elem_Ptr->lookup (neigh_key + i * KEYLENGTH);
              ...
              HashTable * HT_Elem_Ptr)
{
  ...
  for (int i = 0; i < counter; i++)
    {
      element =
        (Element *) HT_Elem_Ptr->lookup (packed_neighbor_info[i].targetkey);
      assert (element);
      which = element->which_neighbor (packed_neighbor_info[i].elkey);
      element->change_neighbor_process (which,
                                        packed_neighbor_info[i].new_proc);
